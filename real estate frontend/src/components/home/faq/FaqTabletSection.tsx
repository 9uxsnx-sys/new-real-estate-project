 ...
 * FAQ Section (Tablet)
 * 
 * Tablet-optimized version with smaller typography
 */
interface FaqTabletSectionProps {
  className?: string;
  lang?: string;
}
...
// Main component
export const FaqTabletSection: React.FC<FaqTabletSectionProps> = ({ 
  className = '', 
  lang = 'en' 
}) => {
  const isRTL = lang === 'ar';
        ...
        <div className="w-full z-[1] flex justify-center">
          <div className="w-full max-w-[600px] flex flex-col gap-4">
            {content.items.map((item, index) => (
              <div 
                key={index}
                className="bg-zinc-100 rounded-[20px] p-5"
              >
                {/* Question (Clickable) */}
                <button
                ...
                >
                  <span 
                    className="text-neutral-900 text-base font-medium group-hover:text-gray-600 transition-colors"
                    style={fontStyle}
                  >
                    {item.question}
                  </span>
