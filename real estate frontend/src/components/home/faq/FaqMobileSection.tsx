 ...
 * FAQ Section (Mobile)
 * 
 * Mobile-optimized version with smaller typography and padding
 */
...
// Main component
export const FaqMobileSection: React.FC<FaqMobileSectionProps> = ({ 
  className = '', 
  lang = 'en' 
}) => {
  const isRTL = lang === 'ar';
  ...
  return (
    <section 
      id="faq"
      className={`w-full bg-white overflow-x-hidden px-4 py-10 ${className}`}
      dir={isRTL ? 'rtl' : 'ltr'}
    >
      {/* Inner Container */}
      <div className="w-full max-w-screen-2xl mx-auto h-min flex flex-col items-center gap-y-8">
        
        {/* Header Section - Mobile scaled */}
        <div className="flex flex-col items-center gap-y-3 w-full max-w-screen-md">
        ...
        <div className="w-full z-[1] flex justify-center px-3">
          <div className="w-full flex flex-col gap-3">
            {content.items.map((item, index) => (
              ...
              >
                ...
                >
                  <span 
                    className="text-neutral-900 text-sm font-medium group-hover:text-gray-600 transition-colors"
                    style={fontStyle}
                  >
                    {item.question}
                  </span>
                ...
                >
                  <p 
                    className="text-zinc-500 text-xs leading-4"
                    style={fontStyle}
                  >
                    {item.answer}
                  </p>
