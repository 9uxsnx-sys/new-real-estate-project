...
export const HeroNavbar: React.FC<HeroNavbarProps> = ({
  ...
  return (
    <nav className={`flex items-center justify-between w-full max-w-screen-2xl mx-auto px-4 md:px-6 lg:px-9 py-3 lg:py-4 ${className}`}>
      {/* Logo */}
      <Link to="/" className="flex items-center gap-x-2 lg:gap-x-3 no-underline">
        ...
        <span 
          className="text-white tracking-[-1.6px] text-xl lg:text-2xl font-semibold"
      ...
      <div className="hidden md:flex items-center gap-x-1 lg:gap-x-2">
        {navItems.map((item) => (
          <Link
            key={item.label}
            to={item.href}
            className="px-3 lg:px-5 py-2 lg:py-3 text-white tracking-[1.6px] uppercase text-xs lg:text-sm font-medium no-underline hover:opacity-80 transition-opacity"
